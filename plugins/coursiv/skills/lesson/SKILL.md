---
name: lesson
description: Expands `%id_kind%` markers inside a Coursiv.io lesson Markdown export (e.g. `%1_q%`, `%5q%`, `%4p%`) into real content, by dispatching each marker to the matching Coursiv sub-skill (currently `question` for quiz markers) and substituting its output in place. Use this whenever the user gives a path to a lesson Markdown file exported from Coursiv.io and wants those `%...%` placeholders filled in, or invokes `/coursiv:lesson`. Trigger it even if they just paste a `.md` path and say something like "process this lesson" or "fill in the questions" — that's exactly this skill's job.
---
...
Markers look like `%<id><kind>%` or `%<id>_<kind>%` — one or more digits (the `id`), optionally followed by an underscore, followed by one or more letters (the `kind`). For example: `%1_q%`, `%5q%`, `%4p%`.
